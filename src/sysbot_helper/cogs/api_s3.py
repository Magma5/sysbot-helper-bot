...
class ApiS3(commands.Cog):
    def __init__(self, bot: Bot):
        self.bot = bot
        self.router = APIRouter(prefix="/api/send_file")
        
        self.router.add_head("/{channel_id:[0-9]+}", self.head_bucket_s3)
        self.router.add_put("/{channel_id:[0-9]+}", self.create_bucket_s3)
        self.router.add_put("/{channel_id:[0-9]+}/{filename:.+}", self.upload_file_s3)
