...
class Helptest(commands.Cog):
    ...
    async def send_pages(self, ctx):
        ...
        button1 = ui.Button(label="Support Server", style=ButtonStyle.link, url=support2)
        ...
        while True:
            try:
                interaction = await ctx.bot.wait_for("select_option", check=None, timeout=30)
