...
_MONTH_NAMES = {
    name: i
    for i, name in chain(enumerate(calendar.day_abbr), enumerate(calendar.day_name))
    if name
}

_DAY_NAMES = {
    name: i + 1
    for i, name in chain(enumerate(calendar.month_abbr), enumerate(calendar.month_name))
    if name
}
...
class CronItem:
    ...
    def __init__(self, item_expr, min_value, max_value, aliases={}):
        ...
        self.aliases = {name.lower(): number for name, number in aliases.items()}
    ...
    def match(self, now: int):
        ...
        """
        return (
            self.range_from <= now <= self.range_to
    ...
    def __parse(self, expr):
        ...
        if "/" in expr:
            ...
            if not interval.isdigit():
                raise ValueError(
                    "Invalid interval: %s in expression %s" % (interval, expr)
                )
        ...
        if self.range_from > self.range_to:
            raise ValueError(
                "Invalid range value: %d-%d in expression %s"
