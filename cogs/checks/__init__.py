...
def is_sudo():
    async def predicate(ctx):
        if not await ctx.bot.is_owner(ctx.author) and not ctx.bot.user_groups().in_group(ctx.author.id, 'sudo'):
...
def is_in_any(*groups):
    async def predicate(ctx):
        if not ctx.bot.user_groups().in_group_any(ctx.author.id, groups):
